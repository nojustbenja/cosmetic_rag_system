...
def test_extract_client_profile_chip_match():
    profile_seca = extract_client_profile("seca", session_history=[])
    assert profile_seca["skin_type"] == "seca"
    profile_sensible = extract_client_profile("sensible", session_history=[])
    assert profile_sensible["skin_type"] == "sensible"

def test_scenario_1_initial_profile_provision():
...
def test_scenario_2_subsequent_turns_the_loop():
    ...
    profile = extract_client_profile(message, session_history=history)
...
def test_scenario_3_missing_profile():
    # Baseline test
